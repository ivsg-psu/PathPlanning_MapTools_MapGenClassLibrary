...
% Grab indices corresponding to the fields
polytopes_field_names = string(fieldnames(polytopes)); 
index_vertices = strcmp(polytopes_field_names(:),'vertices')==1;
index_mean = strcmp(polytopes_field_names(:),'mean')==1;
index_max_radius = strcmp(polytopes_field_names(:),'max_radius')==1;

% Grab all the values out of the structure
polytopes_cells = struct2cell(polytopes);
polytopes_vertices = squeeze(polytopes_cells(index_vertices,1,:));
polytopes_means = squeeze(polytopes_cells(index_mean,1,:));
% polytopes_vertices = cell2mat(squeeze(polytopes_cells(index_vertices,1,:)));
% polytopes_means = cell2mat(squeeze(polytopes_cells(index_mean,1,:)));
polytopes_max_radius = cell2mat(squeeze(polytopes_cells(index_max_radius,1,:)));

% NOTE:
% See: https://www.mathworks.com/matlabcentral/answers/18509-cell-conversion-to-double
% for a way to make the above lines about 4 times faster!

% Scale the polytope in/out based on scale
scale = new_radii_sorted./polytopes_max_radius; % determine scale factor


new_vert = polytopes_means + scale*(polytopes_vertices - polytopes_means);
