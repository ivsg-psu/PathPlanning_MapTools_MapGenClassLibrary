...
%% Split map_name
split_name = split(map_name);

%% Base Map Generation
if sum(split_name=="HST")>0 % Check for Halton set tiling (HST) 
    HST_index = find(split_name=="HST");
    if length(HST_index)>1
...
if sum(split_name=="SMV")>0 % check for shrinking to mean and variance (SMV)
    SMV_index = find(split_name=="SMV");
    if length(SMV_index)>1
