...
for map_layer = 1:map_layers
    ...
    for poly = 1:polys
        ...
        pos = [pos; pos(1,:)]; %#ok<AGROW>

        % add vertices to polytope array
        polytopes_this_layer(poly).vertices = pos; %#ok<SAGROW>
    ...
    polytopes{map_layer} = polytopes_this_layer; %#ok<SAGROW>
end
...
for i = 1:length(polytopes)
    ...
    polytopes_flattened = [polytopes_flattened,polytopes{i}]; %#ok<AGROW>
end

flattened_polytopes = fcn_MapGen_polytopesFlattenMap(polytopes_flattened);
