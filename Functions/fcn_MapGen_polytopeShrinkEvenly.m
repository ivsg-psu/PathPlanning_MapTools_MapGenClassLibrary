function [shrunkPolytope, newVertices, newProjectionVectors, cutDistance] = ...
    ...
    edgeCut,...
    varargin)
% fcn_MapGen_polytopeShrinkEvenly cuts edges off the polytopes
...
% INPUTS:
%
%     unshrunkPolytope: a polytope with fields as defined in
...
% OUTPUTS:
%
%    shrunkPolytope: a shrunken version of the input polytope, where the
%    shrinkage is equally cut from all sides of the polytope. Note: the
%    number of vertices may change due to the cutting process.
%
...
% - Fixed bug where preallocation of polytope structure was producing
...
shrunkPolytope = unshrunkPolytope;

try
    shrunkPolytope.vertices = final_vertices;
catch
    warning('backtrace','on');
    warning('Unable to perform assignment of scalar into a structure array. Throwing error.');
    error('Could not assign single values into array of structure. Was polytopeShrinkEvenly called when polytopesShrinkEvenly was intended?')
end
